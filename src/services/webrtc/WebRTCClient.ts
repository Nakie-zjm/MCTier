...
export class WebRTCClient {
  ...
  /**
   * Ask the backend for this session's chat signing public key.
   *
   * The private half stays in the backend; only this value is published. It is
   * cached so reconnects re-register the same key rather than silently rotating
   * it, which would invalidate the roster every other peer already holds.
   */
