...
impl HostsManager {
    ...
    ///
    /// Windows 使用受限 Helper，Unix 使用 polkit（`pkexec`）请求固定 hosts 操作。
    ///
    /// Unix 通过匿名管道传递受限请求；特权端重新校验 hosts 快照与条目白名单。
    fn write_hosts_file(path: &std::path::Path, content: &str) -> Result<(), AppError> {
        ...
        #[cfg(unix)]
        {
            crate::modules::unix_hosts_helper::write_hosts(path, content)
